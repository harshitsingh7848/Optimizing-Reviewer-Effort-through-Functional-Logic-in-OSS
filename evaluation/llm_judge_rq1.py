...
def compute_effort_scores(records: list) -> list:
    return [float(r.get("total_comments", 0)) for r in records]
...
def evaluate(input_path: str = "pr_dataset_django.json",
    ...
    gt_lookup = {
        pr.get("pr_id"): 1 if effort_values_all[i] >= effort_q80 else 0
        for i, pr in enumerate(dataset)
    }

    print(f"Dataset size: {len(dataset)} PRs [{input_path}]")
    print(f"High effort threshold (Q80): {effort_q80:.4f}")
    ...
    for model_name in MODELS:
        ...
        all_metrics[model_name] = {
            ...
            "high_effort_threshold": round(effort_q80, 2),
            "llm_j": {
